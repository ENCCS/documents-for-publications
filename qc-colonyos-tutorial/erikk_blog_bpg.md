...
The depolarizing noise model represents a scenario where the quantum state loses its coherence and becomes a completely mixed state with a certain probability. Mathematically, for a single-qubit state $\rho$, the depolarizing channel $\mathcal{E}_{\text{dep}}$ $\mathcal{E}_{\text{dep}}$ is defined as:
